    ...
    <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
        ...
        @if ($papeleta->justificacion)
            <div class="sm:col-span-2">
                <dt class="text-gray-500 dark:text-ocean-100/50">Justificación</dt>
                <dd class="text-gray-900 dark:text-white">{{ $papeleta->justificacion }}</dd>
            </div>
        @endif
        @if ($papeleta->motivo_rechazo)
            <div class="sm:col-span-2">
                <dt class="text-gray-500 dark:text-ocean-100/50">Motivo de rechazo</dt>
                <dd class="text-red-700 dark:text-red-400">{{ $papeleta->motivo_rechazo }}</dd>
            </div>
        @endif
        @if ($papeleta->causa_finalizacion_sin_retorno)
            <div class="sm:col-span-2">
                <dt class="text-gray-500 dark:text-ocean-100/50">Causa de finalización sin retorno</dt>
...
@if ($papeleta->retorno)
    <div class="glass-card p-6">
        ...
        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            <div>
                <dt class="text-gray-500 dark:text-ocean-100/50">Hora del servidor</dt>
                <dd class="text-gray-900 dark:text-white">{{ $papeleta->retorno->hora_servidor->format('d/m/Y H:i') }}</dd>
            </div>
            <div>
                <dt class="text-gray-500 dark:text-ocean-100/50">Dentro de radio de sede</dt>
            ...
            @if ($papeleta->retorno->marcado_manual)
                <div class="sm:col-span-2">
                    <dt class="text-gray-500 dark:text-ocean-100/50">Marcado manual por falla de conectividad</dt>
            ...
            @if ($papeleta->descuento_refrigerio_minutos)
                <div>
                    <dt class="text-gray-500 dark:text-ocean-100/50">Descuento de refrigerio</dt>
