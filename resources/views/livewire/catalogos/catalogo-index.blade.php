    ...
    <div class="max-w-4xl mx-auto py-10 sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            @foreach ($catalogos as $catalogo)
                <a wire:key="catalogo-index-a-{{ $loop->index }}" href="{{ $catalogo['ruta'] }}" class="glass-card p-5 flex items-center justify-between hover:opacity-90 transition">
                    <span class="font-semibold text-tinta-950 dark:text-white">{{ $catalogo['nombre'] }}</span>
